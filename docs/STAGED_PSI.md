...
可选的目标延伸层由 `experiments/extended_semantic.py` 实现。它先把固定目标拆成说法契约，再生成分阶段判定探针，并在 evidence/world 输出后执行一次定向判定复核。完整契约见 `docs/TARGET_EXTENSION.md`；运行时使用 `--target-extension`。
