...
def _validate(gold, results, arms):
    ...
    for case in gold["cases"]:
        ...
        case_modes[case_id] = _mode(case, default_mode)
        cases[case_id] = case
    resolved_modes = set(case_modes.values())
    if None in resolved_modes and len(resolved_modes) > 1:
        raise ValueError("mixed or missing assessment_mode in gold cases")
    matrix = {}
    for row in results:
        ...
        row_mode = _mode(row)
        if row_mode != case_modes[case_id]:
            raise ValueError(
                f"{key}: assessment_mode mismatch or missing; "
                f"gold={case_modes[case_id]!r}, result={row_mode!r}")
    ...
    return cases, matrix, case_modes
...
def score(gold, results, bootstrap_samples=10000, seed=2026, arms=ARMS):
    ...
    if not isinstance(bootstrap_samples, int) or isinstance(bootstrap_samples, bool) or bootstrap_samples < 1:
        raise ValueError("bootstrap_samples must be a positive integer")
    cases, matrix, case_modes = _validate(gold, results, arms)
    modes = set(case_modes.values())
    mode = next(iter(modes)) if len(modes) == 1 else "mixed"
    mode_counts = Counter(value or "unspecified" for value in case_modes.values())
    ...
    return {
        ...
        "assessment_mode_case_counts": dict(sorted(mode_counts.items())),
        ...
        "interpretation": [
            "Assessment modes must agree within each case. Aggregate metrics combine the per-case modes shown in assessment_mode_case_counts.",
