...
PROBE_KINDS = ("semantic_core", "polarity", "time_boundary", "quantity_unit",
               ...
               "exact_designation", "designation_relation", "conditional_relation",
               "comparison_relation", "causal_relation", "source_lineage",
               "source_independence")
PLAN_SCHEMA_VERSION = "decision-probe-v3"
...
EXTENSION_SCHEMA = _object(
    ...
    probes=_array(_object(
        ...
        decision_impact=_string(900)), 44),
    notes=_string(1200, 0),
)
...
entity_identity and time, do not put two dimensions of the same kind in one clause. Each time
dimension must be one independently checkable, non-overlapping qualifier; never merge time
qualifiers attached to different entities or inputs into one span. Register only
...
the corresponding dimension appears. Whenever exact_designation appears, also include one
designation_relation probe for that claim and ask whether the complete directed naming or
renaming relation—actor, action, prior referent, asserted designation and qualifiers—is
established together rather than assembled from separately true facts. When target logic is
conditional, comparison or causal, also include the matching relation probe and ask whether
the complete directed relation—not merely its separately true parts—is established. Include
source_independence for world assessment. Reference only supplied
...
time_boundary, entity_identity or exact_designation probe for every matching dimension ID.
Never bind two time dimensions to one time_boundary probe. An entity_identity
question tests referential equivalence and must allow an exact mention, alias, unambiguous
description or anaphora; absence of identical surface wording alone is not a failure. Python
will replace identity and designation question wording with a canonical question derived only
from the target anchor and its match policy, so do not add actor, action or predicate-role
requirements to those questions. An exact_designation question tests the asserted name/title
itself. Bind a relation probe to all
dimensions of its one retained relational claim. This all-dimension rule also applies to
designation_relation. Claim-wide source probes use an empty
...
Create designation_relation ONLY for a claim containing exact_designation, once per such
claim, even when the top-level logic is single, and bind it to every dimension in that claim.
...
def _required_probe_bindings(claim, assessment_mode, logic="single"):
    ...
    if by_kind.get("exact_designation"):
        required.add(("designation_relation", tuple(sorted(
...
class TargetPlanner:
    ...
    def _assemble_claims(target, signature, raw):
        ...
        for item in raw["claims"]:
            ...
            repeated = sorted(kind for kind, anchors in by_kind.items()
                              if kind not in {"entity_identity", "time"} and len(anchors) > 1)
            if repeated:
            ...
            for index, left in enumerate(time_anchors):
                for right in time_anchors[index + 1:]:
                    if max(left.start, right.start) < min(left.end, right.end):
                        raise _StructureRepairNeeded(
                            item["quote"],
                            "Time dimensions overlap inside one clause. Keep each independently "
                            "checkable time qualifier in one non-overlapping anchor; do not merge "
                            "qualifiers attached to different entities or inputs."
                        )
            identity_anchors = [
    ...
    def _assemble_probes(target, signature, claims, raw, logic):
        ...
        for item in raw["probes"]:
            ...
            if match_policy in {"same_referent", "exact_designation"}:
                question = _canonical_identity_question(
                    owned_dimensions[dimension_ids[0]].anchor, match_policy)
            ...
            probes.append(DecisionProbe(identifier, item["claim_id"], item["kind"],
                dimension_ids, question, item["decision_impact"], match_policy,
                routes, gate))
